...
class VaultsManager {
  ...
  static final cipher = AesCbc.with256bits(macAlgorithm: MacAlgorithm.empty);
  ...
  static void loadVaults() {
   ...
   static Future<Vault> createVaultFromPolicy(VaultPolicy policy) async {
    ...
    final cryptKey = SecretKey(passwordToCryptKey(policy.vaultPassword));
    ...
    SecretBox encryptedWitnessFile = await cipher.encryptString(await rootBundle.loadString('file_settings/encryption_witness_file', cache: false), secretKey: cryptKey);
    File witnessFile = File(p.join(path, '.witness'));
    witnessFile.createSync(recursive: true);
    witnessFile.writeAsBytes(encryptedWitnessFile.cipherText);
    ...
    mapFile.writeAsStringSync((await encryptMap(createdVault, constructMap(createdVault)))!);
  ...
  static Future<bool> testVaultKey(Vault vault) async {
    ...
    String decryptedWitnessFile = await cipher.decryptString(SecretBox(witnessFile.readAsBytesSync(), nonce: Uint8List(32), mac: Mac.empty), secretKey: vault.encryptionKey!);

    debugPrint(decryptedWitnessFile);
  ...
  static Future<String?> encryptMap(Vault vault, Map<String, dynamic> initialMap) async {
    if(vault.encryptionKey == null) return null;

    return String.fromCharCodes((await cipher.encryptString(jsonEncode(initialMap), secretKey: vault.encryptionKey!)).cipherText);
  }

  static Future<Map<String, dynamic>?> decryptMap(Vault vault, String encryptedMap) async {
    if(vault.encryptionKey == null) return null;

    Map<String, dynamic> decryptedMap = jsonDecode(await cipher.decryptString(SecretBox(encryptedMap.codeUnits, nonce: Uint8List(32), mac: Mac.empty), secretKey: vault.encryptionKey!));
...
List<int> passwordToCryptKey(String password) {
  final passwordBytes = crypto.md5.convert(utf8.encode(password)).toString().codeUnits;
