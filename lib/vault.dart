...
class VaultsManager {
  ...
  static void loadVaults() {
   ...
   static Future<Vault> createVaultFromPolicy(VaultPolicy policy) async {
    ...
    SecretBox encryptedWitnessFile = await cipher.encrypt(utf8.encode(await rootBundle.loadString('file_settings/encryption_witness_file', cache: false)), secretKey: cryptKey, nonce: Uint8List(cipher.nonceLength));
    ...
    mapFile.writeAsBytesSync((await encryptMap(createdVault, constructMap(createdVault)))!);
  ...
  static Future<bool> testVaultKey(Vault vault) async {
    ...
    String decryptedWitnessFile = utf8.decode(await cipher.decrypt(SecretBox(witnessFile.readAsBytesSync(), nonce: Uint8List(cipher.nonceLength), mac: Mac.empty), secretKey: vault.encryptionKey!), allowMalformed: true);
  ...
  static Future<List<int>?> encryptMap(Vault vault, Map<String, dynamic> initialMap) async {
    if(vault.encryptionKey == null) return null;

    return (await cipher.encrypt(utf8.encode(jsonEncode(initialMap)), secretKey: vault.encryptionKey!, nonce: Uint8List(cipher.nonceLength))).cipherText;
  }

  static Future<Map<String, dynamic>?> decryptMap(Vault vault, List<int> encryptedMap) async {
    if(vault.encryptionKey == null) return null;
    debugPrint(encryptedMap.toString());
    Map<String, dynamic> decryptedMap = jsonDecode(utf8.decode(await cipher.decrypt(SecretBox(encryptedMap, nonce: Uint8List(cipher.nonceLength), mac: Mac.empty), secretKey: vault.encryptionKey!)));
